...
{      //<< operator tulterhelese, hogy kiirja a ketfele objektumot
    ...
    {
        Rect *robj = ((Rect *) obj);
        cout << "Szin: " << (int) obj->get_color() << " Bal felso sarok(u,v):" << robj->get_u() << "," << robj->get_v()
             << " a: " << robj->get_a() << " b: " << robj->get_b();
    }
    if (obj->get_type() == C)        //Circle objektumra vonatkozo
    {
        Circle *cobj = ((Circle *) obj);
        cout << "Szin: " << (int) obj->get_color() << " Kozeppont(u,v):" << cobj->get_u() << "," << cobj->get_v()
             << " Sugar: " << cobj->get_r();
    }
    return os;
}
